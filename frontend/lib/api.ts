...
// This value is baked into the build at compile time via Dockerfile ARG
// Railway passes NEXT_PUBLIC_API_URL as a build argument
...
export const getPipelineRecs = (id: number) =>
  api.get<RecommendationReport>(`/pipelines/${id}/recommendations`).then(r => r.data);

export const updateHealingConfig = (
  id: number,
  config: { self_heal_enabled: boolean; max_retries: number }
) => api.patch(`/pipelines/${id}/healing`, config).then(r => r.data);

export const getHealingLogs = (pipelineId: number) =>
  api.get<HealingLog[]>(`/pipelines/${pipelineId}/healing`).then(r => r.data);
