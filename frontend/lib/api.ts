...
api.interceptors.request.use(config => {
  config.baseURL = `${getBackend()}/api/v1`;
...
export interface RiskAssessment {
  ...
  confidence: number; factors: RiskFactor[]; recommendation: string; based_on_runs: number;
}
export interface Recommendation {
  id: string; title: string; description: string; action_steps: string[];
  priority: string; effort: string; impact: string; category: string; applies_to_stage: string | null;
}
export interface RecommendationReport {
  pipeline_id: number; run_id: number | null; recommendations: Recommendation[];
  summary: string; total_count: number; p1_count: number; generated_from: string;
}
export interface HealingLog {
  id: number; pipeline_id: number; run_id: number; new_run_id: number | null;
  action: string; reason: string; result: string | null;
